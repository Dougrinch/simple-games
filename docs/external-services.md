...
На этапе 1 эти правила безопасно закрывают игровые записи. После реализации
транзакций `database.rules.json` будет расширен и повторно проверен эмулятором
перед production-развёртыванием. Деплой через CLI перезаписывает правила,
указанные в Firebase Console, поэтому источником истины должен оставаться файл
из репозитория.
